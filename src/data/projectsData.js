...
const projects = [
    ...
    },
    {
        ...
        img: '/Projects/aria-ai.webp',
        title: 'Aria — Meridian Estates',
        desc: "An AI property-inquiry assistant for the Cebu market, where the visitor's contact details never enter the model's context. Built as a Next.js 15 App Router app on Neon Postgres: a Gemini flash chat engine grounded by pgvector RAG over the listings table with tool-calling, output guardrails, and a per-session turn budget; a consent-gated capture path that posts contact details straight to the database — enforced independently at the button, the API, and a CHECK constraint — so PII bypasses the LLM entirely under RA 10173; a Postgres outbox that delivers each lead to an agent via Resend under after(), retried by a claim-and-drain worker so a provider outage costs a retry rather than a customer; and a trilingual front end (English, Tagalog, Cebuano) that carries the visitor's chosen language into the system prompt so the assistant opens in it.",
        live: 'https://aria-property-assistant.vercel.app',
        repo: '',
        decisions: [
            'Chose a consent-gated path that writes contact details straight to Postgres over letting the model collect them, so PII never enters the prompt — enforced independently at the button, the API, and a CHECK constraint under RA 10173.',
            'Chose a Postgres outbox drained by a claim-and-retry worker over sending the lead email inline, so a Resend outage costs a retry rather than a customer.',
            'Chose pgvector RAG over the listings table instead of stuffing the catalog into the prompt, so answers stay grounded to real inventory and the per-session turn budget holds.',
            'Chose an eval harness that scores real model replies — and reports provider fallbacks as unscored — over mocked responses, because a mocked LLM can’t fail an injection-resistance test.',
        ],
        metrics: [
            { label: 'unit tests', value: '178' },
            { label: 'eval cases', value: '15' },
            { label: 'languages', value: '3' },
            { label: 'pii in prompt', value: 'none' },
        ],
    },
    {
        slug: 'sugboway',
    ...
    },
    {
        ...
        img: '/Projects/HabiCheck-new.webp',
        title: 'HabiCheck',
    ...
    },
    {
        ...
        live: 'https://v0-prop-hunt-estate.vercel.app',
        repo: '',
    },
    {
        slug: 'rozu-arcade',
    ...
    },
    {
        ...
        img: '/Projects/Discord-telegram-bot.webp',
